...
int main()
{
	...
	cout << "Введите необходимую степень вершины: \n";
	int power;
	int sum = 0;
	cin >> power;

	if (power == 0)
		cout << "ну ноль так ноль";
	else if (power > (n - 1))
		cout << "Такого не бывает!";
	else if (power < (n - 1) && power != 0)
	{
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < n; j++)
			...
			if (sum == power)
			{
				for (int l = 0; l < n; l++)
				{
					if(matrix.at(i).at(l) == 1)
						cout << "ребро между вершиной " << i << "и вершиной " << l << endl;
				}
			}
			sum = 0;
		}
	...
	RenderWindow window(VideoMode(1000, 1000), "Homework 4");
